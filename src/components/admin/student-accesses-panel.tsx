...
import {
  ...
  deleteStudentLessonPackageAction,
...
const LEGACY_NO_SOURCE_LABEL = "Без источника (архивное)";
...
function matchesStudentAccessSearch(
...
function matchesStudentRequestSearch(
...
function matchesSourceFilter(
  ...
  return sourceFilter === "all" || access.school_id === sourceFilter;
}

function matchesDebtFilter(access: StudentAccessCrm, debtFilter: string) {
  const debtAmount = access.crm?.debtAmount ?? 0;
...
function getPackageDescription(
  ...
  const source = packageItem.school?.name ?? LEGACY_NO_SOURCE_LABEL;
  const category = getBookingCategoryLabel(packageItem.booking_category);
  const totalLimit = packageItem.total_lesson_limit
    ? `${packageItem.usedCount ?? 0} из ${packageItem.total_lesson_limit}`
    : `${packageItem.usedCount ?? 0} без лимита`;
  ...
  if (packages.length === 0) {
    return (
      <div className="space-y-3">
        <div className="rounded-xl border bg-white p-3 text-sm">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              <p className="font-semibold">Основной доступ</p>
              <p className="text-muted-foreground mt-1 text-xs">
                {access.school?.name ?? LEGACY_NO_SOURCE_LABEL} · Обычное ·{" "}
                {access.total_lesson_limit
                  ? `${access.total_lesson_limit} занятий`
                  : "без лимита"}{" "}
                ·{" "}
                {access.weekly_lesson_limit
                  ? `до ${access.weekly_lesson_limit} в неделю`
                  : "без недельного лимита"}
              </p>
            </div>
            <Badge
          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            {access.lesson_type_ids.length > 0 ? (
              lessonTypes
                .filter((lessonType) =>
        ...
        <div className="rounded-xl border border-dashed bg-white px-3 py-4 text-sm text-zinc-500">
          После применения миграции дополнительные доступы будут храниться
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              <p className="font-semibold">
                {index === 0 ? "Основной доступ" : "Дополнительный доступ"}
              </p>
              <p className="text-muted-foreground mt-1 text-xs">
            ...
            >
              {packageItem.is_active ? "Активен" : "Отключён"}
            </Badge>
          </div>
          <div className="mt-2 flex flex-wrap gap-1.5">
            {(packageItem.lessonTypes ?? []).map((lessonType) => (
              <span
                key={lessonType.id}
                className="inline-flex items-center gap-1.5 rounded-full bg-zinc-100 px-2 py-1 text-xs font-semibold text-zinc-700"
              >
...
function EditPrimaryStudentAccessForm({
  access,
  lessonTypes,
  schools,
}: {
  ...
  return (
    <details className="rounded-xl border bg-zinc-50">
      <summary className="cursor-pointer list-none px-3 py-3 text-sm font-semibold">
        Редактировать основной доступ
      </summary>
      <form action={formAction} className="space-y-4 border-t px-3 py-4">
        <input type="hidden" name="student_access_id" value={access.id} />
        <input type="hidden" name="display_label" value={access.display_label} />
        <input type="hidden" name="login" value={access.login} />
        <input
          type="hidden"
          name="student_phone"
          value={access.student_phone ?? ""}
        />

        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-2">
            <Label htmlFor={`primary-school-${access.id}`}>
              Автошкола / источник
            </Label>
            <select
            ...
            >
              {!access.school_id && (
                <option value="">
                  {LEGACY_NO_SOURCE_LABEL}, выберите источник
                </option>
              )}
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="space-y-2">
            <Label htmlFor={`primary-weekly-${access.id}`}>
              Лимит в неделю
            </Label>
            <Input
              id={`primary-weekly-${access.id}`}
        ...
        <Button type="submit" variant="outline" disabled={isPending}>
          <Pencil />
          {isPending ? "Сохраняем…" : "Сохранить основной доступ"}
        </Button>
      </form>
    </details>
  );
...
function EditStudentLessonPackageForm({
  packageItem,
  lessonTypes,
  schools,
}: {
  ...
  return (
    <details className="rounded-xl border bg-zinc-50">
      <summary className="cursor-pointer list-none px-3 py-3 text-sm font-semibold">
        Редактировать доп. доступ
      </summary>
      <form action={formAction} className="space-y-4 border-t px-3 py-4">
        <input
          type="hidden"
          name="student_lesson_package_id"
          value={packageItem.id}
        />

        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-2">
            <Label htmlFor={`edit-package-school-${packageItem.id}`}>
              Автошкола / источник
            </Label>
            <select
            ...
            >
              {!packageItem.school_id && (
                <option value="">{LEGACY_NO_SOURCE_LABEL}</option>
              )}
              {editableSchools.map((school) => (
                <option key={school.id} value={school.id}>
                  {school.name}
          ...
          <div className="space-y-2">
            <Label htmlFor={`edit-package-category-${packageItem.id}`}>
              Категория записи
            </Label>
            <select
        ...
        <div className="space-y-2">
          ...
          <LessonTypeCheckboxes
            lessonTypes={editableLessonTypes}
            selectedIds={packageItem.lesson_type_ids}
          />
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-2">
            ...
            <Input
              id={`edit-package-total-${packageItem.id}`}
              name="total_lesson_limit"
              type="number"
              min={1}
              max={500}
              defaultValue={packageItem.total_lesson_limit ?? ""}
            />
          </div>

          <div className="space-y-2">
            <Label htmlFor={`edit-package-weekly-${packageItem.id}`}>
              Лимит в неделю
            </Label>
            <Input
              id={`edit-package-weekly-${packageItem.id}`}
              name="weekly_lesson_limit"
              type="number"
              min={1}
              max={50}
              defaultValue={packageItem.weekly_lesson_limit ?? ""}
            />
          </div>
        </div>
        ...
        <label className="flex items-center gap-2 text-sm font-medium">
          <input
            type="checkbox"
            name="is_active"
            defaultChecked={packageItem.is_active}
            className="size-4"
          />
          Доступ активен
        </label>
        ...
        <Button type="submit" variant="outline" disabled={isPending}>
          <Pencil />
          {isPending ? "Сохраняем…" : "Сохранить доп. доступ"}
        </Button>
      </form>
    </details>
  );
...
function DeleteStudentLessonPackageForm({
  packageItem,
}: {
  packageItem: NonNullable<StudentAccessCrm["packages"]>[number];
}) {
  ...
  return (
    <details className="rounded-xl border border-red-100 bg-red-50/70">
      <summary className="cursor-pointer list-none px-3 py-3 text-sm font-semibold text-red-700">
        Удалить доп. доступ
      </summary>
      <form action={formAction} className="space-y-3 border-t border-red-100 px-3 py-4">
        <input
          type="hidden"
          name="student_lesson_package_id"
          value={packageItem.id}
        ...
        <p className="text-xs text-red-800">
          Удаление доступно только если по этому доп. доступу ещё не было
          записей. Если записи уже есть, доступ лучше отключить.
        </p>
        <StateMessage state={state} />
        <Button
          type="submit"
        ...
        >
          ...
          {isPending ? "Удаляем…" : "Удалить доп. доступ"}
        </Button>
      </form>
    </details>
  );
...
function AddStudentLessonPackageForm({
  access,
  lessonTypes,
  schools,
}: {
  ...
  return (
    <details className="rounded-xl border bg-white">
      ...
      <form action={formAction} className="space-y-4 border-t px-3 py-4">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-2">
            <Label htmlFor={`package-school-${access.id}`}>
              Автошкола / источник
            </Label>
            <select
            ...
            >
              {activeSchools.length === 0 && (
                <option value="">Сначала добавьте источник в настройках</option>
              )}
          ...
          <div className="space-y-2">
            <Label htmlFor={`package-category-${access.id}`}>
              Категория записи
            </Label>
            <select
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="space-y-2">
            <Label htmlFor={`package-weekly-${access.id}`}>
              Лимит в неделю
            </Label>
            <Input
              id={`package-weekly-${access.id}`}
              name="weekly_lesson_limit"
              type="number"
              min={1}
              max={50}
        ...
        <Button type="submit" variant="outline" disabled={isPending}>
          <Plus />
          {isPending ? "Добавляем…" : "Добавить доп. доступ"}
        </Button>
      </form>
    </details>
  );
  ...
  return (
    <details className="rounded-2xl border border-zinc-200 bg-white shadow-sm transition open:border-zinc-500 open:bg-zinc-50/70 open:shadow-md">
      ...
      <div className="space-y-5 border-t px-4 py-5 sm:px-5">
        ...
        <section className="space-y-3 rounded-2xl border border-amber-100 bg-amber-50/50 p-3">
          <p className="text-sm font-semibold">Финансы и история</p>
          <div className="grid gap-3 md:grid-cols-4">
            <div className="rounded-xl bg-white px-3 py-2">
              <p className="text-xs text-zinc-500">Источник</p>
              <p className="truncate font-semibold">
                {access.school?.name ?? LEGACY_NO_SOURCE_LABEL}
              </p>
            </div>
            <div className="rounded-xl bg-white px-3 py-2">
        ...
        <section className="space-y-3 rounded-2xl border border-emerald-100 bg-emerald-50/50 px-3 py-4">
          <div>
            <p className="text-sm font-semibold">Доступы к занятиям</p>
            <p className="text-muted-foreground mt-1 text-xs">
              Основной доступ задаёт обычные занятия ученика. Дополнительный
              доступ добавляйте только когда ученик докупил занятия или перешёл
              на другой источник оплаты.
            </p>
          </div>
          <StudentLessonPackageList
            access={access}
        ...
        <section className="space-y-3 rounded-2xl border border-red-100 bg-red-50/50 p-4">
          <p className="text-sm font-semibold text-red-900">Архив и удаление</p>
...
function StudentSearchPanel({
  ...
  return (
    <section className="rounded-2xl border border-sky-200 bg-sky-50/80 p-4 shadow-sm sm:p-5">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end">
        <div className="min-w-0 flex-1 space-y-2">
          ...
          <div className="relative">
            ...
            <Input
              ...
              placeholder="Имя, логин, способ связи или источник"
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:w-[460px]">
          <div className="space-y-2">
            <Label htmlFor="student-source-filter">Источник</Label>
            <select
          ...
          <div className="space-y-2">
            <Label htmlFor="student-debt-filter">Долги</Label>
            <select
              id="student-debt-filter"
              value={debtFilter}
              onChange={(event) => onDebtFilterChange(event.target.value)}
              className={`${selectClassName} bg-white`}
            >
              <option value="all">Все ученики</option>
              <option value="with_debt">Есть долг</option>
      ...
      <div className="mt-3 flex flex-col gap-2 text-sm text-sky-900 sm:flex-row sm:items-center sm:justify-between">
        <p>
          Найдено:{" "}
          <span className="font-semibold">
...
export function StudentAccessesPanel({
  ...
  const filteredAccesses = accesses.filter(
    (access) =>
      matchesStudentAccessSearch(access, searchQuery) &&
      matchesSourceFilter(access, sourceFilter) &&
      matchesDebtFilter(access, debtFilter),
  );
  const filteredArchivedAccesses = archivedAccesses.filter(
    (access) =>
      matchesStudentAccessSearch(access, searchQuery) &&
      matchesSourceFilter(access, sourceFilter) &&
      matchesDebtFilter(access, debtFilter),
  ...
  const filteredPendingRequests = pendingRequests.filter((request) =>
    matchesStudentRequestSearch(request, searchQuery),
  );
  const currentResultCount =
    tab === "active"
      ...
      : tab === "pending"
        ...
        : filteredArchivedAccesses.length;
  const currentTotalCount =
    tab === "active"
      ? accesses.length
      : tab === "pending"
        ? pendingRequests.length
        : archivedAccesses.length;
      ...
      <section className="rounded-2xl border bg-white p-4 shadow-sm sm:p-5">
        ...
        {tab === "active" ? (
          accesses.length === 0 ? (
            ...
            <div className="rounded-2xl border border-dashed px-4 py-10 text-center text-sm text-zinc-500">
              По этим условиям учеников не найдено.
            </div>
          ) : (
            <div className="space-y-3">
          ...
          pendingRequests.length === 0 ? (
            ...
            <div className="rounded-2xl border border-dashed px-4 py-10 text-center text-sm text-zinc-500">
              По этому поиску заявок не найдено.
            </div>
          ) : (
            <div className="space-y-3">
          ...
          ) : filteredArchivedAccesses.length === 0 ? (
            <div className="rounded-2xl border border-dashed px-4 py-10 text-center text-sm text-zinc-500">
              По этим условиям архивных учеников не найдено.
            </div>
          ) : (
            <div className="space-y-3">
              {filteredArchivedAccesses.map((access) => (
                <ArchivedAccessCard
                  key={access.id}
                  access={access}
                  lessonTypes={lessonTypes}
