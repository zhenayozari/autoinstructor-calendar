...
export default async function DirectorSchedulePage({
  ...
  const timezone =
    selectedInstructor?.timezone ?? instructors[0]?.timezone ?? DEFAULT_TIMEZONE;
  ...
  if (postgresBackend) {
    ...
    loadError = loadError ?? scheduleDayError ?? lessonTypeError ?? schoolError;
  }
        ...
        ? await queryRows<Booking>(
            `
              select id, slot_id, student_label, student_access_id,
                     created_at::text as created_at, price_amount, paid_amount,
                     is_paid, paid_at::text as paid_at, payment_note,
